...
def generate_launch_description():
    ...
    arg_parameters = DeclareLaunchArgument(
        'parameters',
        default_value=PathJoinSubstitution([pkg_lynx_motor_driver, 'config', 'single_test.yaml']),
    )

    arg_can_bus = DeclareLaunchArgument('can_bus', default_value='can0')
    ...
    launch_file_ros2_socketcan_receiver = PathJoinSubstitution(
        [pkg_ros2_socketcan, 'launch', 'socket_can_receiver.launch.py']
    )

    launch_file_ros2_socketcan_sender = PathJoinSubstitution(
        [pkg_ros2_socketcan, 'launch', 'socket_can_sender.launch.py']
    )
    ...
    multi_bldc_group_action = GroupAction(
        actions=[
            ...
            IncludeLaunchDescription(
                ...
                launch_arguments=[('interface', can_bus), ('to_can_bus_topic', [can_bus, '/tx'])],
            ),
            Node(
                package='lynx_motor_driver',
                executable='lynx_motor_driver',
                parameters=[parameters, {'can_bus': can_bus}],
                output='screen',
            ),
        ]
    )
